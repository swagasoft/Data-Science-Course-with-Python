-- First, we needed to group by the day and channel. Then 
-- ordering by the number of events (the third column) gave us a quick way to answer the first question.
...
-- Here you can see that to get the entire table in
--  question 1 back, we included an * in our SELECT statement. You will need to be sure to alias your table.
SELECT *
...
-- hen we just want the region with the max amount from this table.
...
-- For the account that purchased the most (in total over their lifetime as a customer) standard_qty
--  paper, how many accounts still had more in total purchases?
WITH t1 AS (
...
-- For the customer that spent the most (in total
--  over their lifetime as a customer) total_amt_usd, how many web_events did they have for each channel?
